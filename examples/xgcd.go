...
func GCDDiffRec(a int, b int) int {
	if b > a {
		a, b = b, a
	}
	...
	return GCDDiffRec(b, a-b)
}

func GCDDiffIter(a int, b int) int {
	for {
		if b > a {
			a, b = b, a
		}
...
func GCDModRec(a int, b int) int {
	if b > a {
		a, b = b, a
	}
	...
	return GCDModRec(b, a%b)
}

// -----------------------------------------------------------------------------

func GCDModIter(a int, b int) int {

	if b > a { // can move outside loop because will not change!
		a, b = b, a
	}
	for {
		if b == 0 {
