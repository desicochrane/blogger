...
### fermats
https://mathlesstraveled.com/2017/12/21/fermats-little-theorem-proof-by-group-theory/
...
https://rjlipton.wordpress.com/2009/08/07/fermats-little-theorem-for-matrices/
https://www.geeksforgeeks.org/fermats-little-theorem/
http://math453spring2009.wikidot.com/chapter-2
...
3 cases: (5 cases if consider that one could be zero!)
0)a = b gcd(a,b) = a
1) a < b, gcd(a,b) = gcd(a,b-a)
2) b < a gcd(a,b) = gcd(b, a-b)

notice after subtracting a from b, there are 3 cases:
0)remaining is 0 and we are done.
1)remaining is less than a ? this is the same as b mod a
2)remaining is greater than a? can repeat on the remainder. repeat q times until case 1


## KEY IDEA: if m|a and m|b then m|(as+bt) re-used again and again.
## KEY IDEA: gcd(m,0) is m where m != 0
## KEY IDEA: a mod b is at most a/2 (to prove it is "fast" AND to prove it will reach base case of zero)

b = aq+r
above notice that r = b - aq. but that is just a linear combination of a and b, therefore if d|a and d|b then d|r

therefore can see that gcd(a,b) = gcd(a,b-a*q) = gcd(a,r)

-> can apply other identities to make algorithm faster (book)
